...
const containerInputNewProfile = document.getElementById(
  "containerInputNewProfile"
);
...
pdfFileInput.addEventListener("change", (e) => {
  ...
  if (!file) {
    return console.log("no se selecciono file");
  }
...
function handleFile(file) {
  ...
  if (!isPdf) {
    return console.log("el archivo no es PDF");
  }
...
function buildName(profile, originalName, includeDate) {
  if (!profile) {
    return "no seleccionaste un perfil";
  }
  if (!originalName) {
  ...
  let newName = sanitizeName(profile);

  if (!includeDate) {
    return "el nuevo nombre es: " + newName + ".pdf";
  }
...
renameBtn.addEventListener("click", () => {
  ...
  return (statusMessage.textContent = buildName(
    selectedProfile,
    selectedFilePath,
    includeDate
  ));
